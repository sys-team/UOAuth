create or replace procedure ua.authMailru(
    ...
    into
        ...
        @providerRedirectUrl, @refreshTokenUrl, @accessTokenUrl
    from ua.authProvider ap
        left outer join ua.clientAuthProviderRegData caprd on ap.id = caprd.authProvider
    ...
    insert into ua.mailruLog with auto name select
        @xid as xid,
        @refreshTokenUrl as url,
        @tmp as request
    ;

    set @providerResponse =  mailru.processAuthCode(
        @proxyUrl,
        @refreshTokenUrl,
        @providerClientId,
    ...
    from openxml(@providerResponseXml,'/*:response') with(
        refreshToken long varchar '*:refresh_token',
    ...
    if @providerError is null then
        ...
        set @tmp = string (
            @proxyUrl, '?_address=',  @accessTokenUrl, '&',
